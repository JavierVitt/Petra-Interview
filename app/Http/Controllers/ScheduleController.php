...
class ScheduleController extends Controller
{
    ...
    public function fetchItems(Request $request){
        ...
        try {
            ...
            $interviewer = Interviewer::where('user_id', $user->id)->where('event_id', $eventId)->first();
            if (!$interviewer) {
                return response()->json(['error' => 'Interviewer not found'], 404);
            }
    
            // Fetch available interview schedules
            $schedules = AvailableInterviewSchedule::where('interviewer_id', $interviewer->id)
    ...
    public function updateTime(Request $request){
        ...
        $event = Event::find($eventId);
        $user = User::where('email', $email)->first();
        $interviewer = Interviewer::where('user_id', $user->id)->where('event_id', $eventId)->first()->id;
